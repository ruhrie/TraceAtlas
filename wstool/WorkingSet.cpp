...
namespace WorkingSet
{
    ...
    map<uint64_t,BlockWorkingSet> BlockWorkingSetMap;
   ...
   // map<uint64_t, uint64_t> internalAddressIndexMap;
    ...
    void firstStore(uint64_t addrIndex, int64_t t, bool fromStore, uint64_t kernelIndex)
    {
        ...
        if (option == 3)
        {
            if (fromStore)
            {
                ...
                BlockWorkingSetMap[kernelIndex].outputAddressIndexSet.insert(addrIndex);
                //update the map and the vector, if the address has never shown before
                BlockWorkingSetMap[kernelIndex].internalAddressIndexMap[addrIndex] = BlockWorkingSetMap[kernelIndex].internalMapSize;
                BlockWorkingSetMap[kernelIndex].internalAddressLivingVec.push_back(internalAddress);
                BlockWorkingSetMap[kernelIndex].internalMapSize++;
            }
            // birth from a load inst
            else
            {
                //cout<<"33333"<<endl;
                BlockWorkingSetMap[kernelIndex].inputAddressIndexSet.insert(addrIndex);
                BlockWorkingSetMap[kernelIndex].inputMapSize++;
            }
        }
    ...
    {    
        ...
        if(option == 3)
        {
            if (key == "BBEnter")
            {
                if (BBSet.find(stoul(value, nullptr, 0))!= BBSet.end())
                {
                    SetBBCount[stoul(value, nullptr, 0)]++;
                }
            ...
            for (auto &it: BBSet)
            {
                if (SetBBCount[it] > 0)
                {
                    ...
                    else if (key == "LoadAddress")
                    {
                        ...
                        if (BlockWorkingSetMap[it].internalAddressIndexMap.find(addressIndex) != BlockWorkingSetMap[it].internalAddressIndexMap.end())
                        {
                            BlockWorkingSetMap[it].internalAddressLivingVec[BlockWorkingSetMap[it].internalAddressIndexMap[addressIndex]].deathTime = timing;
                            //remove the address from output set, if there is a load corresponding to a store
                            BlockWorkingSetMap[it].outputAddressIndexSet.erase(addressIndex);
                        }
                        else if (BlockWorkingSetMap[it].inputAddressIndexSet.find(addressIndex) == BlockWorkingSetMap[it].inputAddressIndexSet.end())
                        {
